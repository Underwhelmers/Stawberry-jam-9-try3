function ecs_setup_action006_approach() {
    var syst = ecs_setup_system_multiplayer_action(
        "approach {name1}",
        [["position", "sharing_room", "!is_in_reach", "!is_player", "!is_floating", "!is_location"], ["is_player", "position", "!engaged"]],
        function(entities) {
			...
			if (Components.types.proximity.exists_on(thing)) {
				var near = thing.proximity.everyone;
				for (var i = 0, count = array_length(near); i < count; i++) {
